...
class fINd_msgText:
    ...
    def toChannelMsgStr(self, userName: str):
        MsgStr = ''
        if self.lang == 'RU':
            MsgStr = f"&#129302 fINd-Info-bot-OKX запущен в: " \
                     f"{datetime.now().strftime('%H:%M:%S - %Y-%m-%d')}\n" \
                     f"Пользователем: <b>@{userName}</b>"
        if self.lang == 'EN':
            MsgStr = f"&#129302 fINd-Info-bot-OKX is started at: " \
                     f"{datetime.now().strftime('%H:%M:%S - %Y-%m-%d')}\n" \
    ...
    def introMsgStr(self, userFirstName: str):
        ...
        if self.lang == 'RU':
            MsgStr = f"Привет, <b>{userFirstName}</b>!\n\n&#129302 Я <b>fINd-Инфо-бот</b>. " \
                     f"В <b>автоматическом режиме</b> отслеживаю открытые позиции и заявки" \
                     f" на площадке <u>{self.TradingPlatform}</u>" \
                     f" и рассчитываю уровень риска позиций по <b>Фьючерсам</b>"
        if self.lang == 'EN':
            MsgStr = f"Hi, <b>{userFirstName}</b>!\n\n&#129302 I'm <b>fINd-Info-bot</b>. " \
                     f"<b>Automatically</b> monitor open positions and orders " \
                     f"at <u>{self.TradingPlatform}</u> trading platform " \
                     f"and calculate position's risk level of <b>Futures</b> "
        return MsgStr
