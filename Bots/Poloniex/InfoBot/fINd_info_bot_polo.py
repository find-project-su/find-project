# Poloniex Info Bot    | 2024-02-18 | ver. 0.1.0
import telebot
...
from fINd_Text_polo import fINd_msgText, fINd_btnText
from fINd_poloRq import pubPoloRq, privatePoloRq
...
def ReadConf():
    ...
    try:
        ...
        ConfFile = str(os.path.dirname(os.path.abspath(__file__))) + '/fINd_conf_polo.env'
        ...
        FormExText += Fore.YELLOW + "\nPoloniex Info Bot has been terminated" + Style.RESET_ALL
        print(FormExText)
        exit()
    try:
        ConfMsg = Fore.GREEN + "-------------------------------------------\n"
...
privPoloRq = privatePoloRq(str(fINdConf['api_key']),
...
def main_trade_screen(callback):
    ...
    lastAvgPrice = pubPoloRq.getFuturesTickerBTCUSDT()
    accountBalance = privPoloRq.getFuturesAccountBalance()
    accountPositions = privPoloRq.getFuturesPositions()
    accountOrders = privPoloRq.getFuturesOrders()
    # Hide buttons
    if lastMsgTag[0] == 'start_tradeMsg':
        bot.edit_message_text(fINd_messageText.helpMsgStr(),
                              chat_id=callback.message.chat.id,
    ...
    while True:
        if isTradeRun:
            ...
            if j == 15:
                ...
                accountPositions = privPoloRq.getFuturesPositions()
                accountOrders = privPoloRq.getFuturesOrders()
                accountFills = privPoloRq.getFuturesFills()
